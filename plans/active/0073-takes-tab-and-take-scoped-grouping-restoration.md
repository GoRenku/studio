...
number contract. Any remaining work in this plan must consume that contract and
must not introduce an alternative draft, locked, omitted, manual, or
hierarchical scene-number model. Every later scene-number contract, command,
test, and checklist item in this document is superseded by 0153; only this
plan's shot/take use of the resolved durable scene id remains actionable.
