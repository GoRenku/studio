# Renku Studio Data Model And Storage Architecture

Date: 2026-05-05

Status: exploration

Role: historical architecture exploration

This document has not been accepted as current Renku Studio architecture. It is
preserved as historical design exploration.

## Purpose

This document captures the current architecture direction for Renku Studio's data
...
decision for this slice. The current implementation should use
...
> SQLite is the source of truth for durable Renku Studio metadata and
> relationships.

The filesystem remains essential, but it owns content files and generated media,
not metadata relationships. Project-owned file references stored in SQLite must
use normalized project-relative paths, never absolute local filesystem paths.
...
| Canonical term | Use for | Notes |
| --- | --- | --- |
| Asset | A registered content item in Renku Studio metadata. | SQLite owns its identity, type, owner, status, and relationships. |
...
| Select | A durable choice from available takes or imported options. | Use for the current chosen asset(s), including intermediary references such as character sheets. Selects are changeable, but they are not ephemeral UI state. |
| Production Asset | A selected asset that has been materialized into the clean production handoff tree. | Production assets are copied or moved out of working/take folders so editing and export tools see only intended assets. |
...
Renku Studio should make generation cost visible before and after work runs.

| Canonical term | Use for | Notes |
| --- | --- | --- |
| Budget | A user-defined planned spending limit for a scope. | Example scopes: project, episode, sequence, clip, cast member, supported language, localization level, generation definition. |
...
| Selection | Avoid as a vague catch-all. Use Select for durable chosen takes/assets, Pin for cast favorites, and Binding for scoped usage relationships. |
...
Renku Studio should be built around a shared movie domain core.

The product has several interfaces:

- the Renku Studio UI;
- the local Renku Studio server used by the UI;
...
> UI actions and CLI actions must go through the same `studio-core` command
> handlers.
...
This is important because Renku Studio is expected to collaborate with coding
...
- Keep short single-line display text in SQLite when that is clearer than
  creating tiny files. Store paragraph-length, formatted, or multi-line text as
...
- Allow Markdown frontmatter only when it is useful for the file itself. Do not
  ...
  statuses, owner links, or speculative "maybe useful later" fields there.
- Generation definitions are code-owned system behavior in the current direction,
  not project-authored source folders.
- Do not ask agents to hand-edit system-owned state.
- Mutate metadata only through Renku commands or Renku services.
- Keep `studio-cli` and `renku-studio/server` thin.
...
- Store project-owned file references as a first-class project-relative path
  type, resolved through one core-owned path API.
...
However, for Renku Studio, the project metadata will include many relationships
...
- Use migrations owned by `studio-core`.
...
They do not mean `studio-core` should become an application package.
...
`studio-cli` should remain a separate package.

Technically, the CLI could be placed inside `studio-core` behind another export
...
- `studio-core` remains a reusable library instead of becoming a product binary.
...
### Renku Studio Distribution

Renku Studio should support two distribution methods.
...
The `renku-studio` package should remain the product/application package. It
should produce:
...
```text
browser UI
  imports browser-safe contracts from @gorenku/studio-core
  calls the local Renku Studio server

local server
  imports @gorenku/studio-core/node
...
#### Distribution Method 1: CLI-Launched Renku Studio
...
npm package
  ...
  includes embedded Renku Studio UI/server assets
  user runs:
    renku renku-studio launch
```
...
1. Locate the embedded Renku Studio bundle.
2. Start the local Renku Studio server.
...
- `@gorenku/studio-core` compiled output;
...
```text
build renku-studio
copy renku-studio/dist into the CLI package bundle
copy renku-studio/server-dist into the CLI package bundle
...
new `studio-cli` package's binary is a packaging decision.
...
Renku Studio assets and calls `studio-core` through the local server and CLI
surfaces.
...
- the Renku Studio browser UI build;
- the Renku Studio local server build;
- `studio-core` and Node-side dependencies;
...
1. Start the local Renku Studio server from bundled resources.
2. Open the Renku Studio UI in an Electron window.
...
desktop/
  resources/
    ...
    cli/
      renku or renku-studio entrypoint
```
...
They should not fork the Renku Studio implementation.

Both should consume the same `renku-studio` build outputs and the same
`studio-core` APIs.
...
studio-cli or umbrella renku CLI
  owns terminal commands
  may embed renku-studio build assets for npm launch
  starts/stops the local Renku Studio server

desktop
  owns Electron packaging
  embeds renku-studio build assets
...
- The browser UI should keep importing only browser-safe contracts from
  `@gorenku/studio-core`.
- The local server and CLI may import `@gorenku/studio-core/node`.
...
`studio-core`. It should not leak into browser imports.
...
local project folder
  ...
  local media files
  local working assets and production assets
...
Renku Studio should support two project shapes:
...
Renku Studio may also have an app-local database outside the project.
...
own folder and `.renku/project.sqlite`.
...
1. Store runtime tables in `.renku/project.sqlite`.
...
movie-project/
  .renku/
    project.sqlite     durable project metadata
...
- `project.sqlite` owns durable movie metadata and relationships.
...
Completed task history, completed artifact metadata, selects, pins, reference
bindings, and generation records should be durable project metadata and belong
in `project.sqlite`.
...
> Durable project metadata has one source of truth: `.renku/project.sqlite`.
...
- story briefs;
- visual intent notes;
- character descriptions that need paragraphs or formatting;
- narration scripts;
- research notes;
- narration drafts;
- dialog drafts;
...
No required metadata relationship should live in:
...
Markdown frontmatter is allowed only as optional file-local authoring context.
Most Markdown files should not need frontmatter. Frontmatter must not duplicate
canonical SQLite metadata, relationships, owner links, status, ordering, or file
registration data.
...
- its title;
- its project-relative file path;
- its media type;
...
Generated and authored working assets should live near the thing they belong to.
Production assets should live in a separate clean handoff tree that contains
only the assets intended for editing, export, localization, or composition.
...
```text
Working Assets/Base/Visual Language/001-ottoman-court-miniature/Style Sheets/001-painted-miniature-reference.png
Working Assets/Base/Cast/001-mehmed-ii/Portraits/Takes/001-young-sultan-portrait.png
Working Assets/Base/Cast/001-mehmed-ii/Character Sheets/Selects/001-armor-campaign-sheet.png
Working Assets/Base/Sequences/01-logistics/.../References/Takes/001-foundry-lighting-reference.png
Working Assets/Localization/tr-TR/Sequences/01-logistics/.../Subtitles/Selects/001-standard-subtitles.vtt
Production Assets/Master/Sequences/01-logistics/.../Clips/001-cannon-inspection/video.mp4
Production Assets/Localized/tr-TR/Subtitles/001-cannon-inspection.vtt
```
...
- SQLite stores project-owned file references as `project_relative_path`, not
...
- If a user externally renames a file or folder, validation should report the missing
  path explicitly.
- Selects are durable project choices, not ephemeral UI state.
- Materializing a select into `Production Assets/` should use move-like
  semantics where practical: the selected file should not remain confusingly
  duplicated as both a take and a selected working asset unless the command has a
  clear reason to preserve both.
- Reused production assets, such as music and sound effects, should exist once
  under shared production folders. SQLite should record every clip, scene,
  sequence, or project usage relationship.

This lets users browse the project naturally without making the folder tree the
metadata source of truth.

## Code-Owned Generation Definitions

Generation definitions are code-owned system behavior in the current Renku
Studio direction.

They should not be stored as editable project folders, and the project folder
should not contain `Generation Recipes/` or `Recipe Overrides/` trees.

Examples of generation types remain useful as stable generation keys:
...
The generation key records what kind of generation happened. It does not imply
that the user can edit a project-local generation folder.

Code-owned generation definitions may use:

- system prompts and templates bundled with the app or catalog;
- provider/model configuration owned by the generation system;
- explicit context inputs from SQLite and registered assets;
- user-authored Markdown assets such as briefs, narration scripts, visual
  intent notes, character descriptions, localization glossaries, or subtitles.

SQLite may record generation keys, code/catalog versions, provider runs, task
status, cost records, generation packets, and output asset links.

SQLite should not store prompt bodies or generated media blobs as canonical
large text/binary content.

## Catalog-Level Generation Definitions
...
Renku Studio should reuse that pattern for code-owned generation definitions:
...
Catalog generation definitions are system-owned. They are versioned with the
application or catalog, not copied into the project as editable source.
...
It should reference catalog provider, model, and generation definition
identifiers and record the resolved catalog version or content hash used for
...
- a code-owned generation definition owns the generation flow, prompt template,
  system prompt, provider choice, model choice, and model configuration for one
  generation type;
- context comes from explicit inputs, such as cast records, supported language
  ...
  assets, subtitle tracks, timed transcripts, and user-authored briefs;
- generated prompts are outputs of a generation step and can be saved as
...
- code-owned generation definitions choose values for those parameters unless a
  current feature explicitly models user-adjustable generation settings.

The resolved generation definition may use context from:

- the cast member record in SQLite;
- the supported language record, if the generation definition is language-specific;
...
- subtitle tracks or timed transcripts if the generation definition declares
  those inputs;
- optional sequence or clip context if the generation definition declares those
  inputs.

The generation definition should declare required inputs explicitly.

If required context is missing, validation must fail.

No source file, context value, model config, or prompt input should be silently
substituted.

## Generation Definition References

Code-owned generation definitions may reference bundled prompt templates,
provider configuration, model configuration, and runtime steps.

Those references are system implementation details, not project metadata and not
project-local files.

When generation is validated, `studio-core` should validate the selected
provider/model against catalog definitions, validate model parameters against
the selected catalog model schema, gather declared project context from SQLite
and registered assets, compute relevant content hashes, and then compile a
generation packet.

If a required project context asset, registered asset file, language, cast
member, visual language binding, or other declared input is missing, validation
should fail with a clear structured diagnostic.

## Generation Records

A generation record is a lightweight operational record.

It records that a task generated an output using a code-owned generation
definition.

It does not preserve an immutable copy of every prompt template, generation
step, and model configuration used for that take.
...
```text
Adjust the project context and settings for this generation type.
Generate another take.
Mark the takes you like as selects.
```

Do not make users reason about per-take generation version trees.
...
```text
Which generation type generated this take?
Which code/catalog generation definition was used?
...
It should not promise exact historical reconstruction of generation runtime code
after that code has changed.

If exact history is needed later, Git can provide that in a future feature.

## Generation Packets

Before execution, `studio-core` should compile a generation packet.
...
- resolved generation definition reference.
...
Agents should edit project context assets, not completed generation packets.

In v1, generation packets are primarily execution/debug artifacts.

They are not the product's long-term generation history mechanism.
...
`studio-core` should own path allocation for registered assets.

Project-owned file references should use a first-class `ProjectRelativePath`
...
- resolution against the project folder only through a core-owned helper.
...
Renku Studio should handle this with explicit materialization states.
...
- Commit `.renku/project.sqlite` when using Git versioning for a project.
...
However, Git can become a future mechanism for reconstructing the exact project
context used for a generated take.
...
renku-generation-key: cast.character-sheet
...
```text
What exact project context and generation definition created this take?
What language/localization target was it produced for?
```

by reading the project assets from the recorded commit and pairing them with the
recorded generation definition version.

This avoids inventing a custom per-take generation version tree.

It also keeps the v1 model simple:

- takes remember the generation key and generation definition reference;
- takes do not preserve immutable generation snapshots;
- exact historical reconstruction is a future Git-backed feature, not a v1
  requirement.

## 4. Data Model Discussions

This section will grow as Renku Studio adds more domain areas.
...
Renku Studio should make language variation a central production path.
...
If a generation definition requires a voice variant for a target language and
none exists, validation should fail with a clear error.

## Language-Aware Generation

Localization should use code-owned generation definitions just like the rest of
Renku Studio.

Language-aware generation definitions should declare required inputs explicitly.
...
No generation definition should infer target language from folder names.
...
`studio-core` mutation.

## Budget And Cost

Renku Studio should make spending visible and controllable.
...
- `provider` and `model` in cost records are resolved execution facts, not the
  authored source of truth for generation configuration.
...
Before queueing paid generation, `studio-core` should:

1. Resolve the code-owned generation definition.
...
After execution, `studio-core` should:
...
renku movie cost estimate --generation localization.lipsync-clip --target clip:<clip-id> --language tr-TR
...
mutated through `studio-core`.

## Visual Language

Renku Studio needs a top-level place for artistic direction.
...
- `default_project_relative_path` points to the user-friendly visual language
  folder or category.
...
When Renku Studio compiles inputs for generation, it should gather visual
...
There should be no silent fallback when a generation definition requires visual
language guidance.

If a generation definition requires a visual language profile for character
design or clip generation and none is explicitly bound at the required scope,
validation should fail with a clear error.

If a generation definition allows visual language to be optional, that optional
behavior should be part of its explicit contract, not an accidental default.
...
The important rule is that every visual language mutation maps to a `studio-core`
mutation.
...
- `base_voice_supported_language_id` should be explicit when the cast member has
  a voice profile.
- `default_project_relative_path` points to the user-friendly cast folder.
- The folder path is not identity.
...
A select is a durable project choice from a set of takes or imported options.

This follows editing-room language: review the takes, then mark the useful ones
as selects.

Selects are not ephemeral UI state. Users can change them later, but the current
selects are part of the project record.

Examples:

- choose one generated portrait as the current useful portrait;
- choose one character sheet for campaign armor;
- choose one narration take for a clip;
- choose one subtitle file for a localized version;
- choose one clip video take for the master edit.
...
- `owner_type` and `owner_id` identify the scope of the select, such as project,
  visual language, cast member, sequence, scene, clip, or supported language.
- `select_role` identifies what the select is for, such as portrait,
  character_sheet, narration, video, subtitle, music, or sound_effect.
- Selects may later be materialized into `Production Assets/`.
- Materializing a select into production should avoid leaving the same file
  confusingly duplicated as both a take and a selected working asset.
...
If a generation definition requires a character sheet and the clip has no explicit
...
The cast workspace should use the same code-owned generation model as the rest
of Renku Studio.

Examples of cast generation types:
...
- code-owned generation definitions own prompt templates, provider/model
  selection, model parameters, and generation steps;
- user-authored cast context should live in registered Markdown assets when it
  is paragraph-length or formatted;
- short display fields such as cast member name, kind, role, and short
  description can live in SQLite;
- voice generation should declare the target supported language explicitly;
- validation checks that every required project context asset and registered
  asset file exists and is valid;
- providers should receive inputs only after validation and generation packet
  compilation.

No defaults should be silently invented when required context, configuration, or
generation settings are missing.
...
```text
UI or CLI calls studio-core command
validate cast member exists
resolve code-owned cast.character-sheet generation definition
validate required cast context assets and files exist
estimate cost and check relevant budget policy
compile generation inputs into a generation packet
create generation task in SQLite
execute generation definition
...
get resolved generation definitions for cast workspace tabs
list queued/running/completed cast generation tasks
list cast generation cost estimates and actual costs
```

These should be exposed through `studio-core` projection functions and then
...
The important rule is that every command maps to a `studio-core` mutation.
...
- Which cast asset types are required for v1 generation definitions?
- Which user-adjustable generation settings should exist in the first slice, if
  any?
...
3. Asset table.
4. Asset file table with `project_relative_path`.
...
11. Cast asset table.
12. Select table for durable chosen takes/assets.
...
Renku Studio will use SQLite as the source of truth for durable metadata.
...
The filesystem will store user-friendly content and generated media. Markdown
files, subtitles, transcripts, images, audio, video, and compound folders are
all assets when they are part of the project graph.

Short, single-line display fields can stay in SQLite. Paragraph-length,
formatted, or multi-line text should be stored as Markdown asset files.
Frontmatter is optional file-local authoring context only; it must not duplicate
SQLite metadata or relationships.

Generation definitions are code-owned system behavior, not project-local
editable folders.

The folder structure should be pleasant to browse, but it must not define
relationships.

`studio-core` will own schemas, migrations, validation, mutation commands, and
projections.

`renku-studio/server` and `studio-cli` will be thin adapters over `studio-core`.

Agents should call CLI commands for metadata mutations instead of editing
metadata files.

Agents may directly edit registered content assets, such as Markdown notes,
narration drafts, subtitles, timed transcripts, images, audio, and video, when
the user asks them to edit content. SQLite should register and link those files
by project-relative path and hash, not store their bodies as canonical text.
...
Budget and cost should be top-level domain concepts. Renku Studio should store
...
Casting should support many reusable assets per cast member, durable selects,
and explicit clip-specific reference bindings, rather than one global active
character sheet or portrait. Cast voice design should also support a base
language and localized voice variants.

## Future Cloud Versioning Direction

Renku Studio should remain a local-first production workspace.

The future paid cloud direction should not turn Renku Studio into a traditional
...
- working assets and production assets;
- Markdown content assets;
...
> Renku Studio edits the local working copy.
>
