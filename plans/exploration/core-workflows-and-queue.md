# Renku Studio Core, Micro-Workflows, And Queue Architecture
...
That model produced useful infrastructure, but it is not the right center of gravity for Renku Studio.

Renku Studio should instead be centered on:
...
> Create a new Renku Studio domain core and a new Renku Studio CLI.
>
> Use a new Renku Studio micro-workflow YAML format.
>
> Build a lightweight workflow executor and durable queue for Renku Studio.
>
> Reuse or extract only the parts of the old system that solve concrete Renku Studio problems.
>
...
```text
studio-core/
  package: @gorenku/studio-core

studio-cli/
  package: @gorenku/studio-cli

renku-studio/
  package: @gorenku/renku-studio or renku-studio
```

The preferred name is `studio-core`, not `studio-core`.

Reason:

- the domain is the movie project,
- Renku Studio is one UI over that domain,
- agents and the CLI should interact with the same movie domain model without depending on the UI.

`@gorenku/studio-core` should own:
...
`@gorenku/studio-cli` should be the agent/human command surface over that domain.

`renku-studio` should be the desktop app and local server:
...
They should not define Renku Studio's product model.
...
Some of those concepts are useful internally, but they should not leak into Renku Studio as the primary domain language.

Renku Studio should speak in terms of:
...
Start with `studio-core` and pull pieces only when Renku Studio has a concrete need.
...
- Viewer artifact projection as Renku Studio state.

Decision:

> Borrow concepts and small utilities from `core`.
>
> Do not reuse the old blueprint parser, canonical expander, or global planner as Renku Studio's architecture.
...
This is useful for the old workflow, but it is too coarse and too old-model-specific for Renku Studio.

Renku Studio needs commands like:
...
Implementation-wise, these should belong in a new package such as `studio-cli`.
...
> Build a new Renku Studio CLI surface.
>
> Do not make Renku Studio depend on the current `renku generate` flow.

## Blueprint Decision

Renku Studio should not use existing blueprint YAML for micro-workflows.
...
- agents and users need a workflow format that matches the Renku Studio domain.
...
Renku Studio should instead schedule many small tasks.
...
Renku Studio workflows are execution recipes, not graph programs.
...
This replaces the old planner/runner path for Renku Studio.
...
Renku Studio should not start by importing the old planner/runner design wholesale.
...
Renku Studio does not need to infer a surgical target across a giant graph.
...
> Renku Studio needs local step dirty tracking based on step input hashes.
>
...
Renku Studio needs a durable asynchronous queue.
...
> Renku Studio needs a new queue.
>
...
Renku Studio needs both task state and step state.
...
These states should be owned by the Renku Studio services, not directly edited by agents.
...
Renku Studio should not simply fire every queued task immediately.
...
The Renku Studio runtime should include:
...
It handles the real Renku Studio requirements:
...
## How Renku Studio Uses Providers Without Old Blueprints
...
This means Renku Studio does not need old blueprints to get value from providers.
...
The new Renku Studio CLI should be command-oriented around the new domain.
...
Renku Studio will introduce:

- a new `@gorenku/studio-core`,
- a new `@gorenku/studio-cli`,
- a new Renku Studio micro-workflow YAML format,
...
Renku Studio may reuse or extract:
...
> Keep the new Renku Studio core small and domain-shaped.
>
