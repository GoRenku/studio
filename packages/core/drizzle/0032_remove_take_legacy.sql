...
SET `take_id` = REPLACE(`take_id`, 'scene_shot_video_take_generation_', 'scene_shot_video_take_')
WHERE `take_id` LIKE 'scene_shot_video_take_generation_%';--> statement-breakpoint
...
SET `id` = REPLACE(`id`, 'scene_shot_video_take_generation_', 'scene_shot_video_take_')
WHERE `id` LIKE 'scene_shot_video_take_generation_%';--> statement-breakpoint
UPDATE `media_generation_spec`
SET
	`target_id` = REPLACE(`target_id`, 'scene_shot_video_take_generation_', 'scene_shot_video_take_'),
	`spec_json` = REPLACE(`spec_json`, 'scene_shot_video_take_generation_', 'scene_shot_video_take_')
WHERE `target_kind` = 'sceneShotVideoTake'
	AND `target_id` LIKE 'scene_shot_video_take_generation_%';--> statement-breakpoint
