import type { StudioVideoModelRouteProfile } from '@gorenku/studio-engines';
import type {
...
export function projectShotPlanVideoControls(input: {
...
export function validatedShotPlanVideoParameterValues(input: {
  route: StudioVideoModelRouteProfile;
  model: GenerationModelDescriptor;
  ...
  for (const [name, value] of Object.entries(input.parameterValues)) {
    ...
    if (!declared.has(name) || !field) {
      throw invalidParameter(name, 'is not user-configurable on the resolved video route');
    }
    if (value === null) {
      continue;
    }
...
function editorControl(input: {
  ...
  const value = authored
    ...
    : input.initialValue ??
      configurationValue(input.field.defaultValue) ??
      null;
  if (input.field.allowedValues?.length) {
    return {
      ...
      authored,
      recommended: !authored && input.initialValue !== undefined,
...
function invalidParameter(field: string, reason: string): ProjectDataError {
  return new ProjectDataError(
    ...
    `Shot Plan video generation parameter ${field} ${reason}.`,
  );
}
