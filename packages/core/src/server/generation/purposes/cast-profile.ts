...
export const castProfilePurpose = defineGenerationPurpose({
  ...
  async buildReferenceGuide(context) { return buildReferenceGuide({ context, slots: [{ sectionId: 'source', sectionLabel: 'Source', slotId: 'character-sheet', slotLabel: 'Character Sheet', cardinality: 'one', owner: { kind: 'castMember', id: context.target.id }, roles: ['character-sheet'], initializeFirst: true }] }); },
});
