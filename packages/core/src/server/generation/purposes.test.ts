...
describe('generic generation model listing', () => {
  ...
  it('keeps every purpose target, output, and product-setting contract explicit', () => {
    expect(listGenerationPurposes().map((purpose) => ({
      ...
      purpose('cast.video-character-sheet', 'castMember', 'image', [], [aspectRatio('16:9'), quality('high')], gptImage2),
      purpose('cast.storyboard-character-sheet', 'castMember', 'image', [], [aspectRatio('16:9'), quality('high')], gptImage2),
