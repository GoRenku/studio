...
export async function validateStoryboardLookbook(
  input: ValidateStoryboardLookbookInput
): Promise<LookbookValidationReport> {
  return withSession(input, ({ session, projectFolder, project }) => {
    assertStoryboardDocument(input.document);
...
export async function writeStoryboardLookbook(
  input: WriteStoryboardLookbookInput
  ...
  return withSession(input, ({ session, projectFolder, project }) => {
    ...
    const existing = readLookbookRecordByKind(session, 'storyboard');
...
function assertStoryboardDocument(document: { kind: string }): void {
  if (document.kind !== 'storyboardLookbook') {
    throw new ProjectDataError(
      'CORE_LOOKBOOK_TARGET_KIND_INVALID',
      'Storyboard Lookbook commands require a storyboardLookbook document.'
    );
  }
}
...
function writeReport(
  ...
  return {
    ...
    lookbook: toLookbook(requireLookbookRecordByKind(session, 'storyboard')),
