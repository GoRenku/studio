import { importLookbookImageMedia } from '../../media-generation/purposes/lookbook-image.js';
import type { ImageRevisionDestinationDefinition } from '../destination-definition.js';

export const lookbookImageRevisionDestination = {
  kind: 'lookbookImage',
  async importResult(input) {
    const report = await importLookbookImageMedia({
    ...
    if (!file) {
      throw new Error('Imported Lookbook Image has no image AssetFile.');
    }
    return {
      imported: {
...
} satisfies ImageRevisionDestinationDefinition<'lookbookImage'>;
