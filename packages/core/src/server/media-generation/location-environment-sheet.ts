...
import {
  ...
  insertLocationEnvironmentSheetViewRecord,
...
const FILE_ROLES = [
  ...
  'view_back',
  'view_left',
] as const satisfies readonly LocationEnvironmentSheetFileRole[];

const AZIMUTHS = [
  {
    azimuthDegrees: 0 as const,
    direction: 'front' as const,
    fileRole: 'view_front' as const,
  },
  {
    azimuthDegrees: 90 as const,
    direction: 'right' as const,
    fileRole: 'view_right' as const,
  },
  {
    azimuthDegrees: 180 as const,
    direction: 'back' as const,
    fileRole: 'view_back' as const,
  },
  {
    azimuthDegrees: 270 as const,
    direction: 'left' as const,
    fileRole: 'view_left' as const,
  },
];

type LocationEnvironmentSheetImportFiles = Record<
  LocationEnvironmentSheetFileRole,
  string
>;
...
export interface ImportLocationEnvironmentSheetMediaInput
  ...
  files: LocationEnvironmentSheetImportFiles;
  title?: string;
...
export async function importLocationEnvironmentSheetMedia(
  ...
  return withLocationProjectSession(input, async ({ session, projectFolder, project }) => {
    const location = requireLocationForContext(session, input.locationId);
    const normalizedFiles = normalizeImportFiles(input.files);
    await validateImportSourceFiles(projectFolder, normalizedFiles);
    const now = new Date().toISOString();
    const destinationFolder = await allocateLocationEnvironmentSheetFolder({
      projectFolder,
      locationHandle: location.handle,
      title: input.title ?? path.parse(normalizedFiles.composite).name,
    });
    const importedFiles = await copyLocationEnvironmentSheetFiles({
      projectFolder,
    ...
    const imported = await insertImportedLocationEnvironmentSheet({
      ...
      origin: inferImportOrigin(normalizedFiles),
      idGenerator: input.idGenerator,
      now,
    });
...
async function normalizeSpec(input: {
  ...
  const viewFrame = input.spec.viewFrame ?? '16:9';
  if (viewFrame !== '16:9') {
    throw new ProjectDataError(
      'PROJECT_DATA304',
      `Unsupported location environment sheet view frame: ${viewFrame}.`
    );
  }
...
function rejectRemovedSpecFields(spec: LocationEnvironmentSheetGenerationSpec): void {
  ...
  if (present) {
    throw new ProjectDataError(
      'PROJECT_DATA307',
      `Location environment sheet specs no longer support ${present}.`
    );
  }
}
...
function normalizeImportFiles(
  files: LocationEnvironmentSheetImportFiles
): Record<LocationEnvironmentSheetFileRole, ProjectRelativePath> {
  const normalized = {} as Record<LocationEnvironmentSheetFileRole, ProjectRelativePath>;
  for (const role of FILE_ROLES) {
    const value = files[role];
    if (!value) {
      throw new ProjectDataError(
        'PROJECT_DATA308',
        `Location environment sheet import is missing required file role: ${role}.`
      );
    }
    normalized[role] = normalizeProjectRelativePath(value);
  }
  const seen = new Map<string, LocationEnvironmentSheetFileRole>();
  for (const role of FILE_ROLES) {
    const source = normalized[role];
    const previousRole = seen.get(source);
    if (previousRole) {
      throw new ProjectDataError(
        'PROJECT_DATA309',
        `Location environment sheet import reuses ${source} for both ${previousRole} and ${role}.`
      );
    }
    seen.set(source, role);
  }
  return normalized;
}
...
async function validateImportSourceFiles(
  projectFolder: string,
  files: Record<LocationEnvironmentSheetFileRole, ProjectRelativePath>
): Promise<void> {
  for (const role of FILE_ROLES) {
    const sourcePath = resolveProjectRelativePath(projectFolder, files[role]);
    assertResolvedPathInsideProject(projectFolder, sourcePath);
    await statExistingFile(sourcePath);
    if (!isImagePath(files[role])) {
      throw new ProjectDataError(
        'PROJECT_DATA310',
        `Location environment sheet import file must be an image for role ${role}: ${files[role]}.`
      );
    }
  }
}

async function copyLocationEnvironmentSheetFiles(input: {
  projectFolder: string;
  sourceFiles: Record<LocationEnvironmentSheetFileRole, ProjectRelativePath>;
...
> {
  const copied = [];
  for (const role of FILE_ROLES) {
    const sourceProjectRelativePath = input.sourceFiles[role];
    ...
    const destinationProjectRelativePath = joinProjectRelativePath(
      ...
      `${destinationBaseNameForRole(role)}${extensionForSource(sourceProjectRelativePath)}`
    );
...
async function insertImportedLocationEnvironmentSheet(input: {
  ...
  input.session.db.transaction((tx) => {
    ...
    const assetFileIdsByRole = new Map<LocationEnvironmentSheetFileRole, string>();
    for (const file of input.files) {
      const assetFileId = ids('asset_file');
      assetFileIdsByRole.set(file.role, assetFileId);
    ...
    const sheet = insertLocationEnvironmentSheetRecord(txSession, {
      id: ids('location_environment_sheet'),
      locationId: input.locationId,
      assetId,
      compositeFileId: requireImportedFileId(
        assetFileIdsByRole,
        'composite'
      ),
      now: input.now,
    ...
    for (const [index, azimuth] of AZIMUTHS.entries()) {
      insertLocationEnvironmentSheetViewRecord(txSession, {
        id: ids('location_environment_sheet_view'),
        sheetId: sheet.id,
        azimuthDegrees: azimuth.azimuthDegrees,
        assetFileId: requireImportedFileId(
          assetFileIdsByRole,
          azimuth.fileRole
        ),
        sortOrder: index + 1,
...
function requireImportedFileId(
  fileIdsByRole: Map<LocationEnvironmentSheetFileRole, string>,
  role: LocationEnvironmentSheetFileRole
): string {
  const assetFileId = fileIdsByRole.get(role);
  if (!assetFileId) {
    throw new ProjectDataError(
      'PROJECT_DATA308',
      `Location environment sheet import did not create a file for role ${role}.`
    );
  }
  return assetFileId;
}
...
function inferImportOrigin(
  files: Record<LocationEnvironmentSheetFileRole, ProjectRelativePath>
): string {
  return Object.values(files).some((source) => source.startsWith('generated/media/'))
    ? 'generated'
    : 'imported';
}

function destinationBaseNameForRole(role: LocationEnvironmentSheetFileRole): string {
  switch (role) {
    case 'composite':
      return 'composite';
    case 'view_front':
