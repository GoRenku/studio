...
export async function exportProductionAssets(
  ...
  try {
    const project = readProjectInfo(session);
...
function readProjectInfo(session: DatabaseSession): { id: string } {
...
function assertLocaleExists(session: DatabaseSession, localeId: string): void {
...
function readSelectedAssetRows(session: DatabaseSession): SelectedAssetRow[] {
  return [
...
function readProjectSelectedAssetRows(session: DatabaseSession): SelectedAssetRow[] {
  return session.db
    .select({
      assetId: assets.id,
...
function readSequenceSelectedAssetRows(session: DatabaseSession): SelectedAssetRow[] {
  return session.db
    .select({
      assetId: assets.id,
...
function readSceneSelectedAssetRows(session: DatabaseSession): SelectedAssetRow[] {
  return session.db
    .select({
      assetId: assets.id,
...
function readClipSelectedAssetRows(session: DatabaseSession): SelectedAssetRow[] {
  return session.db
    .select({
      assetId: assets.id,
...
function requiredLocaleTag(session: DatabaseSession, localeId: string): string {
  const row = session.db
