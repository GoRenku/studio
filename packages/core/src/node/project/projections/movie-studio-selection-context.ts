...
export function readMovieStudioSelectionContextProjection(
  session: ProjectDataSession,
  input: { selection: MovieStudioSelection }
): MovieStudioSelectionContextResult {
  try {
...
function countSceneClips(session: ProjectDataSession, sceneId: string): number {
  return session.db
    .select({ id: clips.id })
    .from(clips)
...
function countSequenceClips(session: ProjectDataSession, sequenceId: string): number {
...
function countEpisodeSequences(session: ProjectDataSession, episodeId: string): number {
  return session.db
    .select({ id: sequences.id })
    .from(sequences)
    .where(eq(sequences.episodeId, episodeId))
...
function countEpisodeScenes(session: ProjectDataSession, episodeId: string): number {
...
function countEpisodeClips(session: ProjectDataSession, episodeId: string): number {
  const sequenceRows = session.db
    .select({ id: sequences.id })
    .from(sequences)
    .where(eq(sequences.episodeId, episodeId))
...
function selectionNotFound(selection: MovieStudioSelection): MovieStudioSelectionContextResult {
  return {
    valid: false,
