import { isCanonicalId, type ArtifactEventStatus } from '#core';
    ...
    const factory = createProducerHandlerFactory({
      ...
      invoke: async (args: ProducerInvokeArgs) => {
        ...
        try {
          ...
          // 5. Initialize client and get model in both live and simulated modes.
          ...
          const artifacts = buildArtifactsFromResponse(generation.data, request.produces, {
            ...
            outputDefinitions: extras?.outputDefinitions as
              | Record<string, import('#core').BlueprintOutputDefinition>
              | undefined,
          });
...
function formatFanInPromptValue(value: FanInValue, runtime: ProducerRuntime): string {
  ...
  if (lines.length === 0) {
    throw createProviderError(
      SdkErrorCode.MISSING_FANIN_DATA,
      '[providers.vercel-gateway.prompts] Fan-in array input did not yield any values for prompt variable.',
      { kind: 'user_input' },
    );
  }
  return lines.join('\n');
