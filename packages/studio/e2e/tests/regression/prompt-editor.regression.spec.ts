...
test('keeps rich prompt editing, completion, preview, and read-only geometry reliable', async ({
  ...
  const referenceOneLabel = generationPromptProject.preview.references.find(
  ...
  await openGenerationPreview(page, movieProject.projectName, generationPromptProject.preview);
  ...
  const editor = dialog.getByRole('textbox', { name: 'Media generation prompt' });
  ...
  await expect(completion).toBeVisible();
  await expect(completion).toContainText('Image 1');
  await expect(completion).not.toContainText('reference-only context');
  const implicitOption = completion.getByRole('option').filter({
    hasText: 'implicit model reference',
  });
  await implicitOption.click();
  await expect(editor).toContainText('Image 1');
  await page.keyboard.press('ControlOrMeta+Z');
  ...
  const readOnlyEditor = dialog.getByRole('textbox', { name: 'Media generation prompt' });
...
async function openGenerationPreview(
  page: Page,
  projectName: string,
  preview: import('@gorenku/studio-core/client').MediaGenerationPreviewResource,
): Promise<void> {
