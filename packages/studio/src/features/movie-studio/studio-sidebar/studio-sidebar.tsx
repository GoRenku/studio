...
import type { InspirationResource, StudioSelection } from '@gorenku/studio-core/client';
...
export function StudioSidebar({
  ...
  useEffect(() => {
    let cancelled = false;
    void readInspirationResource(project.project.projectName)
      .then((resource) => {
  ...
  const removeInspirationFolder = async (folderId: string) => {
    await deleteInspirationFolder(project.project.projectName, folderId);
  ...
  return (
    <aside className='h-full min-h-0 rounded-(--radius-panel) border border-sidebar-border bg-sidebar-bg overflow-hidden flex flex-col'>
      ...
      <div className='flex-1 min-h-0 overflow-y-auto p-2 space-y-4'>
        ...
        <StudioSidebarSection
          title='Cast'
          detail={`${project.project.counts.castMembers} members`}
        ...
        <StudioSidebarSection
          title='Locations'
          detail={`${project.project.counts.locations} locations`}
        ...
        <StudioSidebarSection
          title='Props'
          detail={`${project.project.counts.props} props`}
