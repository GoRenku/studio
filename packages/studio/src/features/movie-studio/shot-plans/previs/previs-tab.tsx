...
function PrevisWorkspace({ previs }: { previs: ReturnType<typeof usePrevis> }) {
  ...
  const playback = usePrevisPlayback(generationUrl);
  return <div className='h-full overflow-y-auto p-5'>
    ...
    <PrevisMonitor revision={previs.revision} revisions={previs.resource?.revisions ?? []} generation={previs.generation}
      onRevision={(id) => { playback.pause(); previs.setRevisionId(id); }} onGeneration={(id) => { playback.pause(); previs.setGenerationId(id); }} playback={playback}
      status={previs.error ? 'Previs unavailable' : !previs.resource ? 'Loading Previs…' : undefined} />
    <PrevisLegend revision={previs.revision} />
    <div className='grid h-[340px] grid-cols-[minmax(0,61fr)_minmax(0,39fr)] gap-4'>
      <PrevisCues revision={previs.revision} time={playback.time} duration={playback.duration} activeCue={playback.activeCue} playing={playback.playing} seek={playback.seek} playCue={playback.playCue} />
