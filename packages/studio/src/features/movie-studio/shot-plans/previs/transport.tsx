  ...
  return <div className='mt-2 flex min-h-9 items-center gap-2 px-1'>
    ...
    <div className='relative min-w-0 flex-1'>
      <div className='pointer-events-none absolute inset-x-0 top-1/2 h-4 -translate-y-1/2 overflow-hidden rounded-sm' aria-hidden>
        {previewDuration ? <span className='absolute inset-y-0 left-0 bg-foreground/10' style={{ width: `${previewDuration / timelineDuration * 100}%` }} /> : segments.filter((segment) => segment.duration).map((segment, index) => <span key={segment.clip.id} className={`absolute inset-y-0 border-r border-background/80 ${index % 2 ? 'bg-foreground/15' : 'bg-foreground/5'}`} style={{ left: `${segment.start / timelineDuration * 100}%`, width: `${segment.duration! / timelineDuration * 100}%` }} />)}
      </div>
