...
| Voice Casting        | Casting-owned voice identity, delivery, accent, tempo, texture, emotional range, and locale guidance for a Cast Member. | Lives under Cast Design as creative direction. Provider handles and playable sample assets live in Cast Voice records and Cast Voice Provider Registrations. |
...
| Cast Voice Provider Registration | A provider-specific reusable voice handle for one Cast Voice, such as an ElevenLabs TTS voice id or a Kling `voice_id`. | Stored in `cast_voice_provider_registration` with provider, registration model, external provider voice id, capabilities, and source sample asset. |
