...
| Voice Casting        | Casting-owned voice identity, delivery, accent, tempo, texture, emotional range, and locale guidance for a Cast Member. | Lives under Cast Design as creative direction. Provider voice ids and playable sample assets live in Cast Voice records. |
| Cast Voice           | A Cast Member-owned provider voice reference with a Renku reference name, provider, model, provider voice id, purpose, linked sample asset, and `sampleSource` provenance. | Stored in `cast_voice`. Attach custom files or existing ElevenLabs provider samples through `renku cast voice attach`; remove through `renku cast voice remove`. |
