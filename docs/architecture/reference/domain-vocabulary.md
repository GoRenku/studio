...
| Lookbook                      | A durable project visual direction made from user direction, Inspiration sources, screenplay context, or named references.               | Stored as typed Movie or Storyboard tagged JSON plus relationships. It is not a neutral reference summary.  |
| Movie Lookbook                | A Lookbook that owns cinematic visual-language sections such as palette, lighting, texture, composition, camera, and tone/mood.          | Select one Movie Lookbook for movie, cast, location, and shot-video visual-language guidance.               |
| Storyboard Lookbook           | A Lookbook that owns storyboard drawing-language sections such as style brief, line/finish, value/accent, panel/notation, continuity, and guardrails. | Select one Storyboard Lookbook for `scene.storyboard-sheet`; its Lookbook sheet is the required style reference dependency. |
...
| Cast Voice Sample    | The playable audio Asset linked to one Cast Voice. | Attached to the cast member with role `voice_sample` and stored under `cast/<handle>/voice-samples/`. Sources are custom files, generated samples, or existing ElevenLabs provider samples. |
...
| Cast Asset           | A registered asset associated with a cast member.                           | Examples: portrait, character sheet, costume reference, voice sample, research note. Relationship-scoped `referenceName` and `purpose` describe named references without showing filenames or ids in UI. |
| Cast Character Sheet | A cast image asset used as a reusable design reference for the cast member. | Attached with a video- or storyboard-character-sheet role; generated by `cast.video-character-sheet` or `cast.storyboard-character-sheet`. |
...
| Canonical term        | Use for                                                                                                   | Notes                                                                                                                                                     |
| --------------------- | --------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Asset                 | A registered content item in Renku Studio metadata.                                                       | The `asset` row owns identity, type, origin, and availability. Cast Voice sample origins include `imported`, `generated`, and `elevenlabs_sample`; the Cast Voice `sampleSource` carries detailed provenance. Domain relationship rows own where and why the asset is used.                              |
...
| Generation Type       | A category of generation work, such as `cast.video-character-sheet` or `shot.video-take`. | Use Media Purpose when the work is about producing or importing media for a domain object.                                                                                                                       |
