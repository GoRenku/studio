...
| Cast Voice           | A Cast Member-owned provider voice reference with a Renku reference name, provider, model, provider voice id, purpose, and linked sample asset. | Stored in `cast_voice`. Attach through `renku cast voice attach`; remove through `renku cast voice remove`. |
| Cast Voice Sample    | The playable audio Asset linked to one Cast Voice. | Attached to the cast member with role `voice_sample` and stored under `cast/<handle>/voice-samples/`. |
