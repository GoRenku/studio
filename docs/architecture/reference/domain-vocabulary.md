...
| Scene Shot Video Take Generation | A scene-owned shot-video planning unit for one explicit ordered selection from a Scene Shot List.                                      | Owns selected-shot membership, production settings, and compatibility state for shot-video generation. Final video takes preserve the shot membership at import time. |
