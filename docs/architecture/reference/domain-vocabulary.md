...
| Asset Membership      | The one exclusive owner of an Asset.                                                                      | `asset_membership` maps each Asset to one Project, Cast Member, Location, Sequence, Scene, logical Scene Beat, Lookbook, or Shot. Internal owner keys are not public vocabulary. |
| Canonical Selection   | The optional current image for one selectable owner surface.                                              | Applies only to Cast Profile, Location Hero, Lookbook card image, Shot image, and Scene Beat Storyboard. Stored separately from membership. |
| Generation Reference  | One exact file choice saved in a consuming Generation Spec.                                               | Character Sheets, Location Sheets, Lookbook Sheets, and Dialogue Audio Takes are request-scoped and have no canonical selection. |
