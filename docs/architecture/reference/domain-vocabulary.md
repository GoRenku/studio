...
| Location Sheet                | A full-image production reference board for one Location.                                                                               | Attached to a Location with role `location-sheet`; one asset can show maps, elevations, material references, annotated context, or other location-specific visual guidance. |
...
| Location Design      | A validated location-owned production-design document covering spatial thesis, architecture, set dressing, materials, atmosphere, props, continuity, and Location Sheet guidance. | Stored as tagged JSON history in `location_design`; its current media guidance field is `locationSheetGuidance`. It is not a shot list. |
...
| Production Designer  | The specialist agent role that owns Location fact authoring, Location Design, and Location Sheet handoff. | Uses `renku location` and `renku production-design`; hands media generation to `media-producer`. |
