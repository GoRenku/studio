...
| Project Library      | The application-global directory containing Renku Project folders on one computer.          | Its absolute path is the required `storageRoot` in global config. First-run setup recommends one platform-specific `Renku` directory; it is not a Project. |
...
| Screenplay Analysis           | A validated three-act critique of canonical ordered Scenes with Act segments, optional Scene groups, evidence, and suggested Scenes. | Stored as history through `renku screenplay analyze`; flat Screenplays derive segments while exactly three source Acts provide the required segment membership.      |
| Scene Beats                   | A validated Scene-context narrative breakdown made of the narrative-appropriate number of ordered Beats.                                   | Stored as immutable revisions through `renku screenplay beats`; Beat cardinality is independent from later image batching. |
...
| Shot Plan                     | One Scene-locally numbered director/cinematographer production camera plan for ordered Shots and optional Beat coverage.                    | Remains editable regardless of Run or Asset history. Its Shots own planning-image Assets, never Beat Storyboard images or generated video. |
...
| Storyboard Lookbook           | The sole Beat Storyboard appearance authority: style brief, line/finish, value/accent, and guardrails for any deliberate visual language. | Read directly for `scene.storyboard-sheet`; it has no stored pointer to the Production Lookbook and owns no panel staging or Shot coverage. |
...
| Location World                | A navigable Gaussian splat representing one Location.                                                                                   | Location-owned Asset type `location_world`, media kind `model`, one primary full-resolution SPZ; common selection chooses the World shown in Studio while retaining prior candidates. |
| Scene Storyboard Image        | One pre-production story-alignment image candidate owned by a logical Scene Beat.                                                        | Asset type `scene_storyboard_image`; common selection chooses the Beat's current image. Up-to-four-panel composites are temporary image-only batching and never change the Beat revision. |
...
| Voice Casting        | Casting-owned voice identity, delivery, accent, tempo, texture, emotional range, and locale guidance for a Cast Member. | Lives under Cast Design as creative direction. Playable samples and optional opaque provider identity live in Cast Voice records. |
| Cast Voice           | A Cast Member-owned editorial voice reference with a Renku reference name, purpose, linked sample Asset, default-selection state, and optional opaque provider-owned identity. | Stored in `cast_voice`, with `cast_voice_default` selecting one default per Cast Member. Attach files through `renku cast voice attach`; remove through `renku cast voice remove`. |
| Cast Voice Sample    | The playable audio Asset linked to one Cast Voice. | Cast Member-owned Asset type `cast_voice_sample`, stored directly under `cast/<handle>/`. Sources may be imported, generated from voice direction, or retrieved by a provider Engine. |
...
| Cast Asset           | A registered Asset exclusively owned by a Cast Member.                      | Examples: portrait, character sheet, costume reference, voice sample, research note. Asset-owned `referenceName`, one-line summary, and `tags` describe named references without showing filenames or ids in UI. |
| Cast Character Sheet | A Cast Member-owned image Asset used as a reusable design reference.        | Asset type `character_sheet`; generated by `cast.character-sheet` and chosen explicitly as a provider request input. |
...
Use **selection** for one canonical owner-scoped current Asset. Describe an
exact provider request input as an explicit reference rather than selection. Use
...
| Storyboard Continuity Sheet | An ordinary Character, Location, or Prop Sheet prepared in the current Storyboard Lookbook's visual language. | Uses the existing subject-sheet Asset type and purpose with exact Asset tag `storyboard`; never a separate entity or global selection. |
...
| Canonical term        | Use for                                                                                                   | Notes                                                                                                                                                     |
| --------------------- | --------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Asset                 | A registered content item in Renku Studio metadata.                                                       | The `asset` row owns identity, canonical type, media kind, title, summary, reference name, ordered tags, locale, origin, availability, optional generation provenance, and timestamps. Tags are opaque intended-use strings stored as a non-null JSON list. |
...
| Canonical Selection   | The optional current Asset for one selectable owner surface.                                              | Applies only to Cast Profile, Location Hero, Location World, Prop Hero, Lookbook card image, Shot image, and Scene Beat Storyboard. Stored separately from membership. |
| Generation Reference  | One exact local file choice in a temporary provider request.                                              | Stored as a safe `$file` marker during review and preserved in Asset provenance after attachment. It has no Asset-id presentation contract. |
...
| Take                  | A persisted generated or imported candidate in a focused domain that defines Take behavior.              | The current example is a Shot Plan Dialogue Audio Take; ordinary Asset candidates are not generically called Takes. |
| Select                | The explicit act of choosing one canonical Asset for an owner surface.                                    | Exact provider request references are conversational request inputs, not durable selections. |
...
| Canonical term        | Use for                                                                             | Notes                                                                                                                                                                                                            |
| --------------------- | ----------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Media Purpose         | The project-facing reason media is being made or imported.                          | Example: `lookbook.image`. Core uses it for focused target and attachment ownership; it does not define provider request fields. |
...
| Provider Skill        | Agent guidance for choosing a provider/model and authoring its exact request.        | Owns model indexes, operation guides, prompt guidance, reference placement, and conversational execution workflow. |
| Generation Key        | The stable key identifying a generation type.                                       | Prefer Media Purpose Key for media-producing commands.                                                                                                                                                           |
| Media Generation Review Document | A temporary exact request used for conversational Preview and execution. | Contains provider, model, media kind, prompt, and opaque provider-native request JSON. It is not persisted domain state and has no id or lifecycle. |
| Task                  | A queued or running unit of work.                                                   | Example: generate a character sheet for one cast member.                                                                                                                                                         |
| Media Generation Provenance | Exact safe generation facts stored on one attached Asset.                   | Contains provider, model, media kind, prompt, opaque native request, and optional opaque receipt. It is Asset-level and immutable on copies. |
| Provider Request Id   | A provider-assigned id that may support stateless recovery.                         | Returned by Engines/CLI when available. It is not a Renku Run record. |
