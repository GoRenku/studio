...
| Prop authoring replacement deletes | `prop.delete` is a graph edit in the Prop authoring command. Core rejects the delete when assets or Prop Design documents still depend on the Prop. | No Trash item is created because no media artifact is discarded. Dependency-bearing Props are blocked instead of detached or garbage-collected. |
...
| Scene Beats active/history updates | Creating, resetting, or applying Scene Beats creates immutable revisions and may update active state. Every runtime revision remains readable, and active-state changes do not remove media bytes. | No Trash item is required because revision history is the recovery mechanism. |
