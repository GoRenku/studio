...
| Scene Beat Sheet active/history updates | Writing or applying a Scene Beat Sheet creates history rows and updates active state. Older Beat Sheets remain readable as history, and active-state changes do not remove media bytes. | No Trash item is required because revision history is the recovery mechanism. |
