...
Add a node-only coordination service in `@gorenku/studio-core/node`.
...
export interface StudioCoordinationService {
  ...
  readStudioEvents(input?: ReadStudioEventsInput): Promise<StudioEvent[]>;
  readStudioCurrent(input?: ReadStudioCurrentInput): Promise<StudioCurrent>;
}
```

Implementation details may use smaller internal functions, but callers should
not write JSONL lines by hand.
