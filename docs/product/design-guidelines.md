...
and the focus target as `Focus on …`. Preserve every authored creative string
exactly. Do not interpret handles, Markdown, subjects, or visual intent inside
Shot brief text.

Read-only Shot descriptions keep the generic Markdown editor domain-neutral.
The selectable and copied value remains the exact authored Markdown. Studio
does not parse, decorate, validate, or enrich creative Shot descriptions.

Screenplay Narrative subject previews are driven only by Core-validated Cast,
Location, and Prop reference ranges. They use a square frame for Cast profiles,
a 16:9 frame for Location and Prop heroes, and a quiet text-only preview when
optional media is absent. These interactions never search for matching text or
interpret `@handle` tokens.
